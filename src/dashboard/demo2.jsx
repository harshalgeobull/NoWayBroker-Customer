...
const MyProperties = () => {
    ...
    try {
      ...
      if (response.data.status === 1) {
        const transformed = response.data.data.map((item) => {
          ...
          // Set the status based on the available status field
          if (item.available_status === "Sold") {
            status = "Property Sold out";
          } else if (item.available_status === "Available") {
  ...
  const handleMarkAsSold = async () => {
    ...
    formData.append(
      "available_status",
      selectedProperty.status === "Sold Property" ? "Sold" : "Available",
    );
    ...
    try {
      ...
      if (response.data.status === 1) {
        setShowModal(false);
        // Optionally update the UI state here to reflect the "Sold" status
        const updatedProperties = properties.map((property) =>
          property.id === selectedPropertyId
            ? { ...property, soldOut: true, status: "Sold Property" }
            : property,
        );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.status === 1) {
          ...
          toast.error("Offer already exist!");
        }
      } else {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-2xl p-6 w-96 text-center shadow-lg relative">
            ...
            <h2 className="text-2xl font-semibold">
              {selectedProperty?.status === "Sold Property"
                ? "Sold Property"
                : selectedProperty?.status === "Property Sold out"
                  ? "Property Available"
                  : null}
            </h2>
            ...
            <p className="text-gray-500 mt-2">
              {selectedProperty?.status === "Sold Property"
                ? "Are you sure your property is marked as Sold?"
                : selectedProperty?.status === "Property Sold out"
                  ? "Are you sure your property is still Available?"
                  : null}
            </p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex justify-between items-center px-1">
              ...
              <button
                ...
                onClick={() => {
                  setShowOfferCountModal(false); // FIXED
                }}
              >
                Cancel
              </button>
