...
`;
  ...
  useEffect(() => {
    setFormData((prev) => ({
      ...
      sub_sub_property_type:
        propertyType === "Retail" ? prev.sub_sub_property_type : "",
  ...
  useEffect(() => {
    if (propertyCategory === "Paying Guest" && buildingType === "Residential") {
      setPropertyTypes([
  ...
  useEffect(() => {
    if (propertyCategory === "Rent" && buildingType === "Commercial") {
      setPropertyTypes([
        "Office Space",
        "Retail",
        "Land",
  ...
  useEffect(() => {
    if (propertyType === "1RK/Studio Apartment") {
      setFormData((prev) => ({
        ...prev,
        bhk_type: "1 RK",
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in formData) {
      if (key === "property_category_type") {
        let apiValue = formData[key];

        if (apiValue === "Buy" && buildingType === "Residential") {
          apiValue = "Buy";
        } else if (apiValue === "Buy" && buildingType === "Commercial") {
          apiValue = "Buy";
        } else if (apiValue === "Rent" && buildingType === "Residential") {
          apiValue = "Rent";
        } else if (apiValue === "Rent" && buildingType === "Commercial") {
          apiValue = "Rent";
        } else if (apiValue === "Paying Guest") {
          apiValue = "PG/Co-living";
        }

        form.append("property_category_type", apiValue);
      } else if (key === "property_type") {
        let apiValue = formData[key];

        if (formData[key] === "Plot") {
          apiValue = "Plot/Land";
        }

        if (formData[key] === "Land") {
          apiValue = "Plot/Land";
        }

        if (formData[key] === "Office Space") {
          apiValue = "Office";
        }

        form.append(key, apiValue);
      } else {
    ...
    form.append("conferenceRoom", conferenceRoom);
    // form.append("land_type", landType);
    form.append(
      "office_type",
      propertyType === "Retail"
        ? RetailSubType
  ...
  const validateStepFields = () => {
    ...
    if (activeStep === 2) {
      ...
      //   if (!formData.property_price?.trim()) {
      ...
      if (
        ...
        } else if (!/^\d+$/.test(formData.rent.trim())) {
      ...
      if (
        ...
        if (!formData.custom_deposit_amount?.trim()) {
          errors.custom_deposit_amount = "security deposit amount is required";
        }
      }
  ...
  const hideParkingSection =
    propertyType === "Plot" ||
    propertyType === "Land" ||
    officeSubType === "Co-working Office Space";
    ...
    <>
      <div className="flex flex-col items-center min-h-screen bg-white">
        ...
        <nav className="flex items-center justify-center w-full p-4 text-black bg-gray-200">
          <h1 className="text-xl">Add New Building Name/Society Name/Project Name</h1>
        </nav>
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <button
                              type="button"
                              className="flex items-center justify-center w-8 h-8 text-base font-semibold transition border border-gray-300 rounded-full hover:bg-gray-100"
                              onClick={() =>
                                setFormData({
                                  ...formData,
                                  covered_parking: Math.max(
                                    0,
            ...
            <div className="max-w-5xl p-3 mx-auto mb-3 bg-white border border-gray-300 rounded-xl">
              <div>
                ...
                <p className="mt-1 text-gray-500">
                  ...
                  the property, with dynamically updated checkboxes to represent
                  selected features.
                </p>
                ...
                >
                  ...
                  <div className="flex flex-col items-center p-8 mt-4 border-2 border-gray-300 border-dashed rounded-lg">
                    ...
                    <button
                      ...
                      className="mt-2 px-4 py-2 rounded-md my-bg text-white hover:my-bg"
                    >
                      + Add Photos
                    </button>
                    <p className="mt-2 text-xs text-center text-gray-400">
                      Property listing with more images gets 3x more views.
                      <br />
                      Accepted formats: .jpg, .gif, .bmp, .png | Max size: 50MB
                    </p>
                    <input
                      type="file"
                      accept="image/*"
